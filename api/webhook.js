export default async function handler(req, res) {
      ...
      try {
        ...
        // Negeer berichten ouder dan 30 seconden (360dialog retry protection)
        const msgTime = parseInt(messages.timestamp || '0') * 1000;
              if (Date.now() - msgTime > 30000) return res.status(200).json({ status: 'old' });
        ...
        const from = messages.from;
              ...
              if (type === 'text') {
                        ...
                        if (!mediaId) { await send(from, 'Stuur ook even een tekstbeschrijving van wat je wilt controleren.', DIALOG_KEY); return res.status(200).end(); }
                        const mediaResp = await fetch(`https://waba-v2.360dialog.io/v1/media/${mediaId}`, { headers: { 'D360-API-Key': DIALOG_KEY } });
                        if (!mediaResp.ok) { await send(from, 'Kon je bestand niet verwerken. Probeer opnieuw of stuur je vraag als tekst.', DIALOG_KEY); return res.status(200).end(); }
                        ...
                        claudeMessages = [
                            ...
                            { type: 'text', text: `Je bent juridisch assistent voor Pretty Lawsome. Analyseer dit ${isImage ? 'afbeelding' : 'PDF'} in het Nederlands. ${caption} Geen markdown. Eindig met: Wil je verder? Kies rechtenstudent (25 euro) of advocaat (75 euro).` }
                                  ];
              } else {
